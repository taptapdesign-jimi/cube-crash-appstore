...
describe('Board Transition World themes', () => {
  ...
  test('Robo World composes the supplied front-to-back scene and directional character motion', () => {
    ...
    expect(source).not.toContain('const fighterCombatDurationSeconds');
    ...
    expect(source).not.toContain('const lateFlightVariation = {');
    expect(source).not.toContain('const finaleCrossVariation = {');
    expect(source).not.toContain('const toRightShipLocalTime =');
    expect(source).not.toContain('lateWanderProgress');
    expect(source).toContain('{ time: fighterFlightDurationSeconds, x: lowerShipPostBeamFourEnd.x');
    ...
    expect(source).toContain('{ progress: 0.82, x: xForVisualCenter(exitDirection * safeCrossingRadius * 0.88)');
    expect(source).not.toContain('{ progress: 0.68, x: xForVisualCenter');
    expect(source).toContain('finaleClock.progress >= 0.38');
    expect(source).toContain('resolveRoboFighterFinaleWindow(');
    expect(source).toContain('}, fighterFinaleStart);');
    ...
    expect(uninterruptedFlightBranch).toContain('7 * Math.tanh(velocityX / 240)');
    expect(uninterruptedFlightBranch).toContain('sampleBoardTransitionShipHover(elapsed, bankPhase).bank');
    ...
    expect(source).toContain('const hoverXPercent = 900 / shipWidth');
    expect(source).toContain('const hoverYPercent = 1200 / shipHeight');
    ...
    expect(source).toContain('sampleBoardTransitionShipHover(sceneElapsed - runtime.startDelay, runtime.phaseOffset)');
    ...
    expect(source).not.toContain('const actionSway = sampleRoboAirCombatSway(');
    expect(source).toContain("x: sampleSmoothFlightValue(points, elapsed, 'x')");
    expect(source).toContain("y: sampleSmoothFlightValue(points, elapsed, 'y')");
