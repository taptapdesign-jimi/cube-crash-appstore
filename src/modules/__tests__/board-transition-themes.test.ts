...
describe('Board Transition World themes', () => {
  ...
  test('uses only two named NN bees and removes the regular lower-bee runtime', () => {
    ...
    expect(nnBeeSource).toContain('const rightEntryY = leftEntryY + overlayRect.height * 0.60');
    ...
    expect(nnBeeSource).toContain('const tangentStart = sampleByDistance(runtime.samples, travelledDistance - tangentLookahead)');
    expect(nnBeeSource).toContain('const tangentEnd = sampleByDistance(runtime.samples, travelledDistance + tangentLookahead)');
    expect(nnBeeSource).toContain('pointForestTransitionBeeToward(runtime.beeImage, velocityX, velocityY, deltaSeconds)');
    expect(nnBeeSource).toContain('{ x: -beeWidthPx - 34');
    ...
    expect(nnBeeSource).toContain('const point = sampleByDistance(runtime.samples, travelledDistance)');
