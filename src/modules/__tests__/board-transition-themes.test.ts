...
describe('Board Transition World themes', () => {
  ...
  test('owns fourteen pooled Busy Bees with bottom launch and continuous four-depth flight', () => {
    ...
    const specialBeeSource = source.slice(
      source.indexOf('function startForestTransitionSpecialBee'),
    ...
    expect(source).toContain('const horizontalDirection = Math.abs(velocityX) > 0.08');
    expect(source).toContain('const asset = getJourneyForestBeeAssetForVelocity(horizontalDirection, 0)');
    ...
    expect(source).not.toContain('FOREST_TRANSITION_BEE_COMPLETION_CUSHION_SECONDS');
    ...
    expect(source).toContain("const TRANSITION_SCENE_ENTER_ORDER = [\n  'fence-left',\n  'fence-right',\n  'pine1',\n  'pine4',\n  'pine3',\n  'pine5',\n  'pine2',\n  'hill2',\n  'hill1',\n  'mountain'\n];");
    ...
    expect(source).toContain('const phaseDepth = beePlan?.staysInLowFenceBand ? beePlan.fenceDepth : beePlan?.orbitBehindDepth');
    expect(source).toContain('currentDepth: (beeImage.dataset.forestBeeDepth as ForestTransitionBeeDepth | undefined)');
    expect(source).toContain('const targetDepth: ForestTransitionBeeDepth = plan.exitBehindClouds && elapsed >= phaseThreeLocalStart');
    ...
    expect(source).toContain('const BOARD_TRANSITION_NUMBER_ENTER_START_SECONDS = 0.3');
    expect(source).not.toContain('FOREST_TRANSITION_SPECIAL_BEE_NUMBER_EXIT_LEAD_SECONDS');
    ...
    expect(source).toContain("forestTransitionSpecialBeeTimeline = trackTimeline({\n    paused: true,");
    expect(source).toContain('forestTransitionSpecialBeeTimeline?.play(0)');
    expect(source).toContain('() => startForestTransitionSpecialBee(overlay, digitElements)');
    expect(source).not.toContain('forestTransitionSpecialBeeLaunchTimeline');
    expect(source).toContain('const specialBeeWidthPx = 58 * FOREST_TRANSITION_SPECIAL_BEE_SCALE_MULTIPLIER');
    expect(source).toContain('const specialBeeCount = 4');
    expect(source).toContain('const specialBeeDelaySeconds = 0.34');
    expect(source).toContain('const specialBeeRouteDurationSeconds = 1.89');
    expect(source).toContain('const specialBeeRouteSeparationPx = 60');
    expect(source).toContain('const specialBeeScaleMultipliers = [1, 0.8, 0.6, 0.35] as const');
    expect(source).toContain('const randomizedScaleMultipliers = shuffleSpecialBeeValues(specialBeeScaleMultipliers)');
    expect(source).toContain('[-1.5, -0.5, 0.5, 1.5].map((lane) => lane * specialBeeRouteSeparationPx)');
    expect(source).toContain('const randomizedEntrySides = shuffleSpecialBeeValues([true, true, false, false])');
    expect(source).toContain('const randomizedExitSides = shuffleSpecialBeeValues([true, true, false, false])');
    expect(source).toContain('const randomizedOrbitSweeps = shuffleSpecialBeeValues([0.32, -0.40, 0.48, -0.56])');
    expect(source).toContain('const randomizedRotationBiases = shuffleSpecialBeeValues([-5, -2, 2, 5])');
    expect(source).toContain('Array.from({ length: specialBeeCount }, (_, beeIndex) => {');
    expect(source).toContain('const routeIndex = beeIndex');
    expect(source).toContain('const beeScale = 0.86 * randomizedScaleMultipliers[routeIndex]');
    expect(source).toContain('BOARD_TRANSITION_NUMBER_ENTER_START_SECONDS,');
    expect(source).toContain("beeImage.dataset.forestSpecialBeeRole = 'number-pass-zigzag-exit'");
    expect(source).toContain('const entersFromLeft = randomizedEntrySides[routeIndex]');
    expect(source).toContain('const numberLiftPx = Math.max(firstRect.height, secondRect.height) * 0.20');
    expect(source).toContain('const routeLaneOffsetY = randomizedLaneOffsets[routeIndex]');
    expect(source).toContain('y: numberOrbitCenter.y + Math.sin(orbitStartAngle) * numberOrbitRadiusY');
    expect(source).toContain('const orbitSweepTurns = randomizedOrbitSweeps[routeIndex]');
    expect(source).toContain('const rotationBiasDegrees = randomizedRotationBiases[routeIndex]');
    expect(source).toContain('const exitsNorthWest = randomizedExitSides[routeIndex]');
    expect(source).toContain('x: entersFromLeft ? -specialBeeWidthPx - 20 : overlayRect.width + offscreenMargin');
    expect(source).toContain('x: exitsNorthWest ? -offscreenMargin : overlayRect.width + offscreenMargin');
    expect(source).toContain('const scheduledRouteStartSeconds = beeIndex * specialBeeDelaySeconds');
    expect(source).toContain('specialBeeRenderers.push((elapsedSeconds: number) => {');
    expect(source).toContain('const elapsedSeconds = sharedClock.progress * sharedRouteDurationSeconds');
    expect(source).toContain('specialBeeRenderers.forEach((render) => render(elapsedSeconds))');
    expect(source).toContain('onComplete: () => specialBeeCompleteCallbacks.forEach((callback) => callback())');
    expect(source).not.toContain('forestTransitionSpecialBeeTimeline?.to(clock');
    expect(specialBeeSource).toContain("gsap.quickSetter(beeImage, 'rotation', 'deg')");
    expect(source).toContain('const orbitEntryEnd = 0.24');
    expect(source).toContain('const orbitExitStart = 0.68');
    expect(source).toContain('const exitMidpoint = 0.84');
    expect(source).toContain('const orbitEase = orbitProgress * orbitProgress * (3 - 2 * orbitProgress)');
    expect(source).toContain('orbitDepthSignal = Math.sin(orbitAngle)');
    expect(source).toContain('rotationSetter(renderedBank + rotationBiasDegrees * gentleWobbleEnvelope)');
    expect(source).toContain('const bankLimit = 8');
    expect(source).toContain('const targetBank = Math.max(-bankLimit, Math.min(bankLimit, Math.atan2(velocityY, Math.max(0.01, Math.abs(velocityX))) * 6))');
    expect(source).toContain('const canChangeSpriteDirection = progress < orbitEntryEnd || progress >= orbitExitStart');
    expect(source).toContain('if (canChangeSpriteDirection && nextHorizontalDirection !== 0 && nextHorizontalDirection !== horizontalDirection)');
    expect(source).not.toContain('const verticalBounce =');
    expect(source).toContain("beeImage.dataset.forestSpecialBeeIndex = String(beeIndex)");
    expect(source).toContain("gsap.set(beeImage, { opacity: 0, visibility: 'hidden', display: 'none' })");
    expect(source).toContain("console.info('[CC_FOREST_NN_BEE]', payload)");
    expect(source).toContain("'visible-on-empty-screen'");
    expect(source).toContain('__ccForestNNBeeTrace');
    expect(source).toContain("beeImage.dataset.forestBeeLowFence = String(busyPlan.staysInLowFenceBand)");
    expect(source).toContain("staysInLowFenceBand: beeImage.dataset.forestBeeLowFence === 'true'");
    expect(source).toContain("pineLoopProgress >= 0.22 && pineLoopProgress < 0.72");
    expect(source).toContain("exitBehindClouds: beeImage.dataset.forestBeeLowFence !== 'true' && index % 4 === 1");
    expect(source).toContain("plan.exitBehindClouds && elapsed >= phaseThreeLocalStart\n          ? 'behind-clouds'");
    expect(source).toContain("Object.freeze({ depth: 'behind-clouds', zIndex: 14 })");
    expect(source).toContain("? Math.max(-24, Math.min(24, velocityX * 0.24 + velocityY * 0.10))");
    expect(source).toContain("busyPlan.staysInLowFenceBand\n          ? busyPlan.frontFenceHold ? 'front-of-fence' : 'behind-fence'");
    expect(source).toContain('const gentleWobbleEnvelope = Math.sin(Math.PI * progress)');
    expect(source).toContain('const individualWobble = progress * Math.PI * 2 * (1.15 + beeIndex * 0.08) + beeIndex * 0.7');
    expect(source).toContain('renderedBank += (targetBank - renderedBank) * 0.08');
    expect(source).toContain('scaleXSetter(beeScale * (1 + cartoonStretch))');
    expect(source).toContain('scaleYSetter(beeScale * (1 - cartoonStretch * 0.65))');
    expect(source).toContain('if (progress >= 1) {');
    ...
    expect(source).not.toContain('const beeCompletionSeconds =');
