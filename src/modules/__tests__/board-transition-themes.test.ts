...
describe('Board Transition World themes', () => {
  ...
  test('owns ten pooled 3-4-3 bees with an organic two-stage depth and exit timeline', () => {
    const source = fs.readFileSync(path.resolve(process.cwd(), 'src/modules/board-transition-screen.ts'), 'utf8');
    expect(source).toContain("type ForestTransitionBeeDepth = 'front-of-fence' | 'behind-fence' | 'behind-pine2' | 'behind-pine4'");
    ...
    expect(source).toContain("Object.freeze({ depth: 'behind-pine2', zIndex: 35 })");
    expect(source).toContain("Object.freeze({ depth: 'behind-pine4', zIndex: 35 })");
    expect(source.match(/group: 'left'/g)).toHaveLength(3);
    expect(source.match(/group: 'center'/g)).toHaveLength(4);
    expect(source.match(/group: 'right'/g)).toHaveLength(3);
    expect(source.match(/depth: 'front-of-fence'/g)).toHaveLength(3);
    expect(source.match(/depth: 'behind-fence'/g)).toHaveLength(3);
    expect(source.match(/depth: 'behind-pine2'/g)).toHaveLength(4);
    expect(source.match(/depth: 'behind-pine4'/g)).toHaveLength(4);
    ...
    expect(source).toContain('Math.random() * (FOREST_TRANSITION_BEE_ENTRY_BOTTOM_MAX_PX - FOREST_TRANSITION_BEE_BOTTOM_MIN_PX)');
    expect(source).toContain('const restScale = 0.80 + Math.random() * 0.40');
    expect(source).toContain('beeImage.dataset.forestBeeRestScale = restScale.toFixed(4)');
    expect(source).toContain('const targetLeftPercent = 3 + Math.random() * 94');
    expect(source).toContain('const curvePx = curveDirection * (14 + Math.random() * 10)');
    expect(source).toContain('midX: Math.max(minTravelX, Math.min(maxTravelX, unclampedMidX))');
    expect(source).toContain('midY: Math.max(minTravelY, Math.min(maxTravelY, unclampedMidY))');
    ...
    expect(source).toContain('const laneCenterPercent = ((placementIndex + 0.5) / FOREST_TRANSITION_BEE_PLACEMENTS.length) * 100');
    expect(source).toContain('laneCenterPercent + (Math.random() * 5 - 2.5)');
    expect(source).toContain('const originMode = placementIndex % 3');
    expect(source).toContain('const enterDelay = placementIndex * (0.018 + Math.random() * 0.004)');
    expect(source).toContain('const enterDuration = 0.52 + Math.random() * 0.14');
    expect(source).toContain('forestContainer.clientHeight - bottomPx + renderedHalfWidth + 24');
    expect(source).toContain('resolveForestBeeCubic(enterStartX, controlOneX, controlTwoX, 0, progress)');
    expect(source).toContain("onComplete: () => ownedIdleTimeline.play(0)");
    expect(source).toContain('if (placementIndex % 2 === 1)');
    expect(source).toContain("const orbitTreeKey = placementIndex % 4 === 1 ? 'pine2' : 'pine4'");
    expect(source).toContain('const orbitRadiusX = 48 + Math.random() * 26');
    expect(source).toContain('const orbitRadiusY = 32 + Math.random() * 22');
    expect(source).toContain('const orbitHeightRatio = 0.10 + Math.random() * 0.13');
    expect(source).toContain('const orbitDuration = 2.75 + Math.random() * 0.35');
    expect(source).toContain('const orbitDirection = placementIndex % 3 === 0 ? -1 : 1');
    expect(source).toContain('const orbitSlotCount = orbitTreeKey === \'pine2\' ? 3 : 2');
    expect(source).toContain("const nextDepth = Math.sin(angle) <= 0 ? orbitBehindDepth : 'front-of-fence'");
    expect(source).toContain("duration: orbitDuration,\n        ease: 'none'");
    expect(source).toContain('if (nextDepth !== currentOrbitDepth)');
    expect(source).toContain('const smoothApproach = approachProgress * approachProgress * (3 - 2 * approachProgress)');
    expect(source).not.toContain('forestTransitionBeeTimelines.forEach((timeline) => timeline.play(0))');
    expect(source).toContain('const idleTimeline = trackTimeline({ repeat: -1, paused: true })');
    expect(source).toContain('const returnDuration = 0.80 + Math.random() * 0.40');
    expect(source).toContain('Math.min(((97 - startLeftPercent) / 100) * containerWidth, unclampedReturnMidX)');
    expect(source).toContain('Math.min(bottomPx - FOREST_TRANSITION_BEE_BOTTOM_MIN_PX, unclampedReturnMidY)');
    ...
    expect(source).toContain('const initialDepth = placement.depth');
    ...
    expect(source).toContain('const forestTransitionBeeTimelines = new Map<HTMLImageElement, gsap.core.Timeline>()');
    ...
    expect(source).toContain('const FOREST_TRANSITION_BEE_INTRO_DURATION_SECONDS = 1.5');
    expect(source).toContain('const FOREST_TRANSITION_BEE_PHASE_TWO_AT_SECONDS = 1.5');
    expect(source).toContain('const FOREST_TRANSITION_BEE_PHASE_THREE_AT_SECONDS = 3');
    expect(source).toContain('const FOREST_TRANSITION_BEE_EXIT_COMPLETE_AT_SECONDS = 4.2');
    ...
    expect(source).toContain('FOREST_TRANSITION_BEE_PHASE_TWO_AT_SECONDS,\n  );');
    ...
    expect(source).toContain('const minimumSeparationPx = 60');
    ...
    expect(source).toContain('const upperCount = Math.floor(bees.length * 0.5)');
    expect(source).toContain('const verticalDirections = bees.map((_beeImage, index) => index < upperCount ? -1 : 1)');
    expect(source).toContain('const upperPoints = createEntropyPointSet(upperCount, 0.08, 0.92, 0.10, 0.58, true)');
    expect(source).toContain('1 - 70 / Math.max(1, containerRect.height)');
    expect(source).toContain('1 - 10 / Math.max(1, containerRect.height)');
    expect(source).toContain('const phaseTwoPoints = verticalDirections.map');
    ...
    expect(source).toContain('phaseTwoEnd.x + (phaseTwoEnd.x - phaseTwoArrival.x) * 0.52');
    ...
    expect(source).toContain("layout: 'entropy'");
    expect(source).toContain('upperCount,');
    expect(source).toContain('lowerCount: bees.length - upperCount');
    expect(source).toContain('lowerBottomBandPx: [10, 70]');
    ...
    expect(source).toContain('const exitProgress = Math.max(0, (elapsed - phaseThreeLocalStart)');
    ...
    expect(source).toContain("beeImage.dataset.forestBeeDepth = 'behind-fence'");
    ...
    expect(source).toContain('stopForestTransitionBeeMotion();');
    ...
    expect(source).toContain('forestTransitionBeeTimelines.delete(beeImage)');
    ...
    expect(source).toContain('startForestTransitionBees(forestContainer)');
    expect(source.indexOf('startForestTransitionBees(forestContainer)')).toBeGreaterThan(
      source.indexOf('document.body.appendChild(overlay)'),
    );
    expect(source).toContain("preloadTransitionAssets(showScene ? sceneLayers : [], resolvedTheme === 'forest')");
