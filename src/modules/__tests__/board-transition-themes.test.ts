...
describe('Board Transition World themes', () => {
  ...
  test('owns ten pooled 3-4-3 bees with an organic two-stage depth and exit timeline', () => {
    ...
    expect(source).toContain('FOREST_TRANSITION_BEE_PLACEMENTS.forEach((placement) => {');
    ...
    expect(source).toContain('const groupHorizontalRange: Record<ForestTransitionBeeGroup, readonly [number, number]>');
    expect(source).toContain('const startLeftPercent = groupMinX + Math.random() * (groupMaxX - groupMinX)');
    expect(source).toContain('gsap.set(beeImage, { x: 0, y: 0, opacity: 1, scale: restScale');
    expect(source).toContain('forestTransitionBeeTimelines.forEach((timeline) => timeline.play(0))');
    ...
    expect(source).toContain('const phaseThreeLocalStart = FOREST_TRANSITION_BEE_PHASE_THREE_AT_SECONDS');
