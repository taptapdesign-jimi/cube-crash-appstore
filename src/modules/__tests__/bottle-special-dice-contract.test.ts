...
describe('Bottle special-die visual contract', () => {
  ...
  test('sinks bottles from above with a bounded trailing-bubble stream, then cleans up idempotently', () => {
    ...
    expect(scene).toContain('yPercent: 0');
    expect(scene).toContain('scale: 1');
    expect(scene).toContain('rotationX: 0');
    expect(scene).toContain('rotationY: 0');
    expect(scene).toContain("document.createElement('img')");
    expect(scene).toContain('if (pooledImages.has(image))');
    expect(scene).toContain('const BUBBLE_OPACITY_MIN = 0.2');
    expect(scene).toContain('const BUBBLE_OPACITY_MAX = 0.7');
    expect(scene).toContain('const trailOpacity = trailBubbleOpacities[emissionOrdinal]');
    expect(scene).toContain('opacity: bubbleOpacity');
    ...
    expect(scene).toContain('const bottleRect = image.getBoundingClientRect()');
    expect(scene.match(/field\.getBoundingClientRect\(\)/g)).toHaveLength(1);
    expect(scene).toContain('const origin = getFieldOrigin()');
    ...
    expect(scene).toContain('const trailTimeline = own(trackTimeline({');
    expect(scene).toContain('delay: bottleStartDelaySeconds');
    expect(scene).toContain('paused: true');
    expect(scene).toContain('trailTimeline.call(() => {');
    expect(scene).toContain('trailTimeline.play(0)');
    expect(scene).toContain('bottleTimeline.set(mover, { opacity: 1, scale: BOTTLE_SINK_START_SCALE }, 0)');
    ...
    expect(scene).toContain('bubble.style.zIndex = String(120 + (index % 3))');
    expect(scene).toContain('opacity: trailOpacity');
