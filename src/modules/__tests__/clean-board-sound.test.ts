...
describe('Clean Board result sounds', () => {
  ...
  test('uses the supplied result, counter and earned-star assets', () => {
    ...
    expect(CLEAN_BOARD_APPLAUSE_ACTION_VOLUME).toBe(0.8);
    expect(CLEAN_BOARD_APPLAUSE_VOLUME).toBe(0.48);
    expect(CLEAN_BOARD_SAXOPHONE_HAPPY_SOUND_SOURCE)
      .toBe('./assets/sound/Clean board/saxophone happy.wav');
    ...
    expect(crypto.createHash('sha256').update(saxophoneBytes).digest('hex'))
      .toBe('168c1d682aaf3af6daa9b4950a38fa8fbd3e94eab9a60206dbb8c4d25a8af620');
    expect(CLEAN_BOARD_SAXOPHONE_HAPPY_ACTION_VOLUME).toBe(0.9);
    expect(CLEAN_BOARD_SAXOPHONE_HAPPY_VOLUME).toBe(0.54);
    expect(CLEAN_BOARD_MONEY_COUNT_SOUND_SOURCE).toBe('./assets/sound/Clean board/money count.wav');
    expect(CLEAN_BOARD_MONEY_COUNT_ACTION_VOLUME).toBe(0.8);
    expect(CLEAN_BOARD_MONEY_COUNT_VOLUME).toBe(0.48);
    ...
    expect(CLEAN_BOARD_FAST_POINTS_STACK_ACTION_VOLUME).toBe(0.9);
    expect(CLEAN_BOARD_FAST_POINTS_STACK_VOLUME).toBe(0.54);
    ...
    expect(CLEAN_BOARD_STAR_HARP_ACTION_VOLUME).toBe(0.5);
    expect(CLEAN_BOARD_STAR_HARP_VOLUME).toBe(0.3);
  ...
  test('connects cues to the exact animation starts and only earned filled stars', () => {
    ...
    expect(modal).toContain('stopCleanBoardSounds();');
