...
describe('Kanta special die', () => {
  ...
  test('builds four enlarged collectors and one dense Kanta heap without spaceships', () => {
    ...
    expect(KANTA_FINALE_ROBOT_FIXED_EXTRA_RAISE_RATIOS).toEqual([0.10, 0.15, 0.05]);
    expect(KANTA_FINALE_ROBOT_RANDOM_EXTRA_RAISE_MIN_RATIO).toBe(0.05);
    expect(KANTA_FINALE_ROBOT_RANDOM_EXTRA_RAISE_MAX_RATIO).toBe(0.10);
