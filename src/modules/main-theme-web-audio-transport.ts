...
/** Keeps Arcade loops on the Web Audio clock so iOS cannot insert a media-element restart gap. */
class SoundtrackBufferVoice implements MainThemeVoiceLike {
  loop = true;
  preload: HTMLMediaElement['preload'] = 'auto';
  ...
  get duration(): number { return this.buffer?.duration ?? Number.NaN; }
  get decodedBytes(): number {
    return this.buffer ? this.buffer.length * this.buffer.numberOfChannels * 4 : 0;
  }
  get contextState(): string { return this.context.state; }
  ...
  set currentTime(value: number) {
    const position = this.normalizePosition(value);
    this.storedPosition = position;
    if (!this.isPaused && this.buffer) this.startSource(position);
  }
  get volume(): number { return this.envelope.value; }
  ...
  private normalizePosition(value: number): number {
    ...
    if (!duration || !Number.isFinite(duration) || duration <= 0) return safeValue;
    return this.loop ? safeValue % duration : Math.min(safeValue, duration);
  }
  ...
  private startSource(position: number): void {
    ...
    this.anchorPosition = this.normalizePosition(position);
    this.anchorContextTime = startAt;
