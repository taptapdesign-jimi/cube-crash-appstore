...
export function layout({ app, top }) { 
  ...
  if (HUD_ROOT._hudElements) {
    ...
    if (coin && coin.container) {
      ...
      if (HUD_ROOT._scoreTouchArea) {
        ...
        scoreTouchArea.x = coin.container.x - 51 + 20; // 51px = half of 102px width, +20px right (shifted 4px left from +24px)
  ...
  if (HUD_ROOT._xButton) {
    ...
    const xTopPadding = 2 - 16 - 8 - 4 - 2; // Move up 30px from yValue (2px higher - user requested)
...
export function initHUD({ stage, app, top = 8, initialHide = false }) { 
  ...
  if (coinHud && coinHud.container) {
    ...
    const scoreTouchAreaWidth = 102; // User requested: 94px + 8px = 102px width
