...
import { showWildJuiceBubblesExplosion, stopWildJuiceBubblesExplosion, forceStopWildJuiceBubblesExplosion, isWildJuiceBubblesExplosionActive, isWildJuiceBubblesExplosionRecentlyStarted, waitForBubblesExplosionToComplete, destroyWildJuiceBubblesExplosionCache } from './wild-juice-bubbles-explosion.ts';
...
function buildGameplayRuntimeFlags(options: {
  ...
  const ignoreSkipWindow = options.ignoreSkipWindow === true;
  const skipActive = !ignoreSkipWindow && options.skipWindowExceeded === true;
  const flags: GameplayRuntimeFlags = {
    busyEnding,
    wildSpawnInProgress: wildSpawnInProgress && !skipActive,
    merge6SpawnInProgress: merge6SpawnInProgress && !skipActive,
    wildMagnetPullInProgress: wildMagnetPullInProgress && !skipActive,
...
async function spawnWildFromMeter(){
  ...
  const activeAnimationBlockReason = getWildSpawnAnimationBlockReason();
  if (activeAnimationBlockReason) {
    devLog('⏸️ spawnWildFromMeter skipped - active animation guard:', activeAnimationBlockReason);
    return false;
  }
...
function merge(src: Tile, dst: Tile, helpers: MergeHelpers){
  ...
  if (effSum === 6){
    ...
    let isLastMerge = isFinalMergeByResolver || isRegularRegularLastTwoMerge6 || isAnyWildLastTwo || isWildRegularLastTwo || isLastMergeableTiles || isWildLastTileMerge || isWildMagnetLastTwo;
    ...
    trackTween(src, {
      ...
      onComplete: async () => {
        ...
        if ((isFinalMergeByResolver || wasWildRegularLastTwo || wasRegularRegularLastTwo || wasMagnetRegularLastTwo) && !isLastMergeInOnComplete) {
          (dst as any)._isLastMerge = true;
        ...
        const hasTilesToPullFlag = (dst as any)?._hasTilesToPull === true;
        ...
        const currentFinalBlockersForSourceOfTruth = getFinalMergeTileSets({
          tiles: collectBoardGameplayTiles(),
          src,
          dst,
        }).finalMergeBlockersBefore;
        const isLastMergeFlagSet =
          (isFinalMergeByResolver || currentFinalBlockersForSourceOfTruth.length === 0) &&
          (
            (dst as any)?._isLastMerge === true ||
        ...
        if (isLastMergeFlagSet && !willPulledTilesMerge) {
          ...
          const cleanupFinalGhostsAndLocked = () => {
            try {
              ...
              // Remove all locked tiles/placeholders. Final merge cinematics should never show board residue.
              const lockedToRemove = tiles.filter((t: any) => {
                if (!t || t.destroyed) return false;
                if (t.locked) return true;
                return (t.value | 0) <= 0 && !t.special && t.eventMode === 'none';
              });
          ...
          // Run for every final merge, not only magnet. This prevents one-frame/visible
          // locked placeholder residue while Sparkle/Bubbly/TNT/SWOOP text is playing.
          ...
          const waitForWildFinaleAnimations = async () => {
            ...
            const waitForJuiceFinale = async () => {
              ...
              if (isWildJuiceBubblesExplosionActive?.()) {
                const maxWaitMs = isArcadeHomeRunMode() ? 5200 : 6500;
                devLog(`⏳ Waiting for wild-juice bubbles completion before clean board`, { maxWaitMs });
                await waitForBubblesExplosionToComplete(maxWaitMs);
              } else {
        ...
        const maybeForceCleanBoardFromSingleMerge6 = async (reason: string): Promise<boolean> => {
          ...
          (dst as any)._isLastMerge = true;
          setFinalMergeVisualSuppression(true);
          await triggerCleanBoardFlow(`clean_board_from_spawn_guard_${reason}`);
          return true;
        };
        ...
        const maybeForceCleanBoardFromFinalWildSnapshot = async (reason: string): Promise<boolean> => {
          ...
          cancelPendingWildContinuation(`final_wild_guard_${reason}`);
        ...
        const maybeForceCleanBoardFromFinalRegularSnapshot = async (reason: string): Promise<boolean> => {
          ...
          cancelPendingWildContinuation(`final_regular_guard_${reason}`);
          try { (dst as any)._isLastMerge = true; } catch {}
          try { (src as any)._isLastMerge = true; } catch {}
        ...
        if (shouldSpawnAtDst) {
          ...
          const doEndgameSpawns = async () => {
            if (await maybeForceCleanBoardFromFinalWildSnapshot('endgame_before_spawn')) return;
          ...
          if (isRegularMerge6) {
            ...
            void (async () => {
              if (await maybeForceCleanBoardFromFinalWildSnapshot('wild_normal_before_spawn')) return;
              let mergeCellWild = 0;
              try {
        ...
        const clearMergeCellPlaceholderArtifact = () => {
          ...
          const isLockedPlaceholder = !!atMergeCell.locked && ((atMergeCell.value | 0) <= 0);
          if (!isLockedPlaceholder) return;
          grid[gy][gx] = null;
...
async function tryTutorialFinalChanceSpawn(reason: string): Promise<boolean> {
  ...
  try {
    ...
    checkLevelEndTimer = trackDelayedCall(0.12, () => {
      checkLevelEndTimer = null;
      checkLevelEnd();
    });
    return true;
  ...
  try {
          ...
          if (!spawned) {
            try {
              ...
              if (t) {
                ...
                resetTileToNormalState?.(t);
                if (drag && typeof drag.bindToTile === 'function') drag.bindToTile(t);
                const forcedValue = randomRegularTileValue();
    ...
    if (shouldHandleStuck) {
      ...
      for (const delayMs of stuckConfirmationDelaysMs) {
        ...
        const recheckResolvedLevelEnd = resolveLevelEndDecision({
