...
export async function boot(): Promise<void> {
  ...
  logger.info(`Canvas dimensions: ${canvas.width}x${canvas.height}`, 'app-core');
  console.log('✅ Canvas added to DOM:', canvas.width, 'x', canvas.height);
...
export async function layout(): Promise<void> {
  ...
  // Ensure board is on-screen (no negative positions)
  board.x = Math.max(0, board.x);
  board.y = Math.max(0, board.y);
  
  // Scale down if board is too large for screen
  if (boardDimensions.w > app.screen.width || boardDimensions.h > app.screen.height) {
    const scaleX = app.screen.width / boardDimensions.w;
    const scaleY = app.screen.height / boardDimensions.h;
    const scale = Math.min(scaleX, scaleY) * 0.9; // 90% to leave some margin
    
    board.scale.set(scale);
    console.log('✅ Board scaled down to:', scale);
  }
  ...
  boardBG.clear();
  boardBG.rect(0, 0, app.screen.width, app.screen.height);
  boardBG.fill({ color: 0xF5F5F5, alpha: 1 });
  boardBG.visible = true;
  boardBG.alpha = 1;
  
  // Log board position and dimensions
  console.log('✅ Board position:', board.x, board.y);
  console.log('✅ Board dimensions:', boardDimensions.w, 'x', boardDimensions.h);
  console.log('✅ App screen:', app.screen.width, 'x', app.screen.height);
  console.log('✅ App canvas:', app.view.width, 'x', app.view.height);
  console.log('✅ Stage children count:', stage.children.length);
  console.log('✅ Stage children:', stage.children.map(c => c.constructor.name));
  console.log('✅ Board parent:', board.parent);
  console.log('✅ Board visible:', board.visible, 'alpha:', board.alpha);
...
async function createInitialTiles(): Promise<void> {
  ...
  // Log tiles information
  console.log('✅ Tiles created:', tiles.length);
  if (tiles.length > 0) {
    const firstTile = tiles[0];
    console.log('✅ First tile position:', firstTile.x, firstTile.y);
    console.log('✅ First tile visible:', firstTile.visible);
    console.log('✅ First tile alpha:', firstTile.alpha);
    console.log('✅ First tile parent:', firstTile.parent ? 'has parent' : 'NO PARENT!');
  }
}
