...
- Beach Ball Merge-6 reuses that same complete ordinary four-layer owner unchanged, then chooses exactly one Ball pair per committed event: a 50/50 selection between `ball2.wav` plus `ball4.wav`, both immediately, or `ball1.wav` immediately plus `ball5.wav` at `+100ms`. All four pair layers play at authored `1.0x / 0.6667 action gain / 0.40 effective gain`. `ball impact.mp3` is an additional invariant immediate layer at authored `1.0x / 0.50 action gain / 0.30 effective gain`.
- Beach Ball sound trigger: the committed `effSum === 6` branch only when merge-entry variant provenance identifies `beach-ball`; the variant is selected once per event and Bottle, core Wild Star and all other registered Specials remain excluded
...
- Playback: pickup and rejected-return woosh cues start from `currentTime = 0.045`; ordinary, core Wild Star and Beach Ball Merge-6 start the shared four-layer base from zero with the primary/crash pair at `1.30x`; Wild Star starts one magic layer immediately and sparkle at `+200ms`; Beach Ball starts impact plus one selected pair's primary immediately, with `ball4.wav` also starting immediately or `ball5.wav` starting at exactly `+100ms`
- Ownership: twenty-five bounded preloaded decoded/fallback sources across `gameplay-pickup-sound.ts`, `ordinary-stack-sound.ts`, `regular-merge6-sound.ts`, `wild-star-merge6-sound.ts`, `beach-ball-merge6-sound.ts`, `wild-special-landing-sound.ts`, `arcade-crate-sound.ts` and `journey-backpack-sound.ts`; shared ordinary source buffers are decoded only once, and each Wild/Special event adds only the layers owned by its exact variant
...
- Settings: `gameSoundsEnabled` gates preload and playback; switching OFF or board cleanup stops and rewinds all twenty-five preloaded sources and cancels all delayed sparkle/Ball/crate/backpack fallback timers
